...
def get_car_ids() -> list:
    results = []
    for num in range(0, 110000, 20):  # TODO 범위 바꿔주자
        url_ = "http://api.encar.com/search/car/list/premium"
        param = {
            "count": "true",
            "q": "(And.Hidden.N._.CarType.Y.)",
            "sr": "|ModifiedDate|{}|20".format(str(num))
        }
        resp = requests.get(url_, params=param)
        data = resp.json()
        search_data = data["SearchResults"]
        try:
            for car_info in search_data:
                c_id = car_info["Photo"].split('/')[-1].replace('_', '')
                results.append(c_id)
...
if __name__ == "__main__":
    ...
    sql = """INSERT INTO car_sell (car_id, car_reg_date, car_brand, car_name
                , car_size, car_power
                , car_price, car_kil, car_age) VALUES (:car_id, :reg_date, :brand, :car_name, :car_size, :car_power, :price, :kil, :age) """
